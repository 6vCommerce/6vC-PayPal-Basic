...
<html>
  ...
  <body bgcolor="#FFFFFF" link="#355222" alink="#355222" vlink="#355222" style="font-family: Verdana, Geneva, Arial, Helvetica, sans-serif; font-size: 10px;">
    ...
    [{foreach from=$order->getOrderArticles() key=sOrdItmId item=oOrdItm}]
      <tr>
        ...
        <td style="font-family: Verdana, Geneva, Arial, Helvetica, sans-serif; font-size: 10px; padding-top: 10px;" valign="top" align="right">
          <a href="[{ $oViewConf->getBaseDir() }]index.php?shp=[{$shop->oxshops__oxid->value}]&amp;anid=[{ $oOrdItm->oxorderarticles__oxartid->value }]&amp;cl=review&amp;reviewuserhash=[{$reviewuserhash}]" style="font-family: Verdana, Geneva, Arial, Helvetica, sans-serif; font-size: 10px;" target="_blank">[{ oxmultilang ident="EMAIL_ORDER_CUST_HTML_REVIEW" }]</a>
        </td>
      </tr>
    [{/foreach}]
    <tr>
  ...
  [{if $oViewConf->getShowGiftWrapping() && $order->getGiftCard() }]
  ...
  [{ if $order->oxorder__oxremark->value }]
    <b>[{ oxmultilang ident="EMAIL_ORDER_CUST_HTML_YOURMESSAGE" }] </b>[{ $order->oxorder__oxremark->value }]<br>
    <br>
  [{/if}]
  ...
  [{*if $payment->oxuserpayments__oxpaymentsid->value == "oxidpayadvance"}]
    ...
    [{ oxmultilang ident="EMAIL_ORDER_CUST_HTML_IBAN" }] [{$shop->oxshops__oxibannumber->value}]<br>
    <br>
  [{/if*}]
